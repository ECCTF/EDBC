...
//	In this file, squaring and multiplication are both performed as one multiplication and two additions.

int JacobiMixedAdditionAtomicity(projectPoint P1, affinePoint P2, projectPoint &P3)
{
	big R4=mirvar(0),R5=mirvar(0),A=mirvar(0),B=mirvar(0),M=mirvar(0),C=mirvar(0),D=mirvar(0);//,P3Y=mirvar(0);
	nres_modmult(P1.Z,P1.Z,R5);
	//Big R5=P1.Z*P1.Z%p;//Z_1^2
	nres_modadd(R5, P1.X, C);//Atomicity, it does not affect the final value
	nres_modsub(P1.X, R5, D);//Atomicity, it does not affect the final value
	...
	nres_modadd(A,A, P3.Z);//P3.Z=(2*P1.Z*B)%p;
	nres_modsub(P1.X, R5, D);//Atomicity, it does not affect the final value
	...
	nres_modmult(R5,R5,R5);//R5=(4*B*B)%p;//4B^2
	nres_modsub(P1.X, R5, D);//Atomicity, it does not affect the final value
	nres_modsub(P1.X, R5, D);//Atomicity, it does not affect the final value
	...
	nres_modadd(R5, R5, D);//D=2R5
	nres_modadd(R4, D, C);//C=2R5+D

	nres_modmult(A,A,B);
	//B=A*A%p;
	nres_modsub(B,C, P3.X);
	//P3.X=(B-R4-2*R5)%p;
	nres_modsub(R5, P3.X,R5);
...
int JacobiAdditionAtomicity(projectPoint P1, projectPoint P2, projectPoint &P3)
{
	...
	//Z1Z1=Z1^2
	nres_modadd(Z1Z1, P1.X, C);//Atomicity, it does not affect the final value
	nres_modadd(Z1Z1, P1.X, C);//Atomicity, it does not affect the final value
	...
	//r=S2-S1, V=U1*HH
	nres_modadd(V, V, C);//C=2V
	nres_modadd(C, HHH, D);//D=2V+HHH

	nres_modmult(r, r, A); 
	nres_modsub(A, D, P3.X);
	//X3=r^2-HHH-2V
	nres_modsub(V, P3.X, A);

	nres_modmult(r, A, A);
	nres_modadd(A, D, C);//Atomicity, it does not affect the final value
	nres_modadd(A, D, C);//Atomicity, it does not affect the final value
...
int JacobiAdditionSubtractionAtomicity(projectPoint P1, projectPoint P2, projectPoint &P3, projectPoint &P4)
{
	...
	nres_modmult(P1.Z, P1.Z, Z1Z1);
	//Z1Z1=Z1^2
	nres_modadd(Z1Z1, Z1Z1, C);//Atomicity, it does not affect the final value
	nres_modadd(Z1Z1, Z1Z1, C);//Atomicity, it does not affect the final value
	...
	nres_modmult(H, HH, HHH);
	//HH=H^2, HHH=H*HH	
	nres_modadd(S1, S2, C);//C=S1+S2
	nres_modsub(R4, C, r1);
	//r1=-S2-S1

	nres_modmult(U1, HH, V);//r=S2-S1, V=U1*HH
	nres_modadd(V, V, D);//D=2V
	nres_modadd(HHH, D, B);//B=2V+HHH
...
int JacobiDoubleAtomicity(projectPoint P1, projectPoint &P3)
{
	...
	nres_modadd(A,A,P3.Z);
	nres_modadd(A, A, C);//Atomicity, it does not affect the final value	
...
{
	big A=mirvar(0),B=mirvar(0),C=mirvar(0),D=mirvar(0),mid=mirvar(0),M=mirvar(0), X = mirvar(0);
	nres_modmult(P1.Z,P1.Z, A);
	...
	nres_modadd(B, B, X);//Atomicity, it does not affect the final value	
...
int EdwardsAdditionAtomicity(extProjectPoint P1, extProjectPoint P2, extProjectPoint &P3, big d)
{
	...
	nres_modadd(B, B, H);//Atomicity, it does not affect the final value	
	nres_modadd(B, B, H);//Atomicity, it does not affect the final value	
...
{
	//P2.Z=1,7M
	big C = mirvar(0), B = mirvar(0), E = mirvar(0), H = mirvar(0), I = mirvar(0), J = mirvar(0), A = mirvar(0), D = mirvar(0);
	big F = mirvar(0), G = mirvar(0);
	nres_modsub(P1.Y, P1.X, H);
	//H=Y1-X1
	nres_modsub(P2.Y, P2.X, I);
	...
	nres_modmult(P1.Z, P2.T, J);
	nres_modadd(J, J, C);//C=2Z1*T2
	//nres_modmult(P1.T, P2.Z, H);	
	nres_modadd(D, C, E);//E=D+C
...
int EdwardsDoubleAtomicity(extProjectPoint P1, projectPoint &P3, big d)
{
	...
	nres_modmult(P1.X, P1.X, A);
	//Big A=(P1.X*P1.X)%p;//X_1^2
	nres_modadd(A, A, E);//Atomicity, it does not affect the final value	
	nres_modadd(A, A, E);//Atomicity, it does not affect the final value	
	...
	nres_modmult(I, I, E);
	nres_modsub(E, C, E);	//E=(P1.X+P1.Y)^2-A-B
	nres_modsub(D, B, H);//H=D-B
...
int EdwardsDoubleAtomicity(extProjectPoint P1, extProjectPoint &P3, big d)
{
	...
	//P3.Y=G*H
	nres_modsub(G, C, F);//F=G-C
	nres_modadd(A, A, I);//Atomicity, it does not affect the final value
...
{
	//7M
	big A = mirvar(0), B = mirvar(0), C = mirvar(2), D = mirvar(0), E = mirvar(0), a = mirvar(-1), I = mirvar(0);
	big G = mirvar(0), F = mirvar(0), H = mirvar(0);

	nres_modmult(P1.X, P1.X, A);
	//Big A=(P1.X*P1.X)%p;//X_1^2
	nres_modadd(P1.X, P1.Y, I);	 
	nres_modsub(F, A, D);	//D=aA;

	nres_modmult(P1.Y, P1.Y, B);	//Big B=(P1.Y*P1.Y)%p;//Y_1^2
	//nres_modmult(P1.Z, P1.Z, C);
	nres_modadd(A, B, H);
	nres_modadd(D, B, G);//G=D+B
	...
	nres_modmult(E, F, P3.X);
	//P3.X=E*F
	nres_modadd(A, A, A);//Atomicity, it does not affect the final value
	nres_modadd(A, A, H);//Atomicity, it does not affect the final value
...
int EdwardsTripleAtomicity(projectPoint P1, projectPoint &P3, big d)
{
	...
	nres_modadd(B, B, H);
	nres_modadd(B, B, A);//Atomicity, it does not affect the final value
	...
	//Y_1^2
	nres_modadd(YY, aXX, Ap);	//Ap=YY+aXX
	nres_modsub(H, Ap, F);

	


	nres_modmult(aXX, F, xB);
	nres_modadd(xB, xB, xB);
	//xB=aXX*B
	nres_modadd(F, F, B);
	//B=2(2Z1^2-Ap)
	...
	nres_modmult(Ap, F, AA);//AA=Ap*(YY-aXX)
	nres_modsub(AA, yB, F);	//F=AA-yB
	nres_modadd(yB, AA, H);
...
int EdwardsMixedTripleAtomicity(projectPoint P1, projectPoint &P3, big d)
{
	...
	//xB=aXX*B
	nres_modadd(xB, xB, xB);
	nres_modadd(F, F, B);	//B=2(2Z1^2-Ap)
...
int EdwardsMixedTripleAtomicity(extProjectPoint P1, projectPoint &P3, big d)
{
	...
	nres_modmult(P1.Y, P1.Y, YY);
	//Y_1^2
	nres_modsub(C, P1.X, B);
	//-X_1
	nres_modadd(B, B, A);//Atomicity, it does not affect the final value

	nres_modmult(P1.X, B, aXX);
	//aX_1^2
	nres_modadd(YY, aXX, Ap);
	//Ap=YY+aXX
	//nres_modmult(P1.Z, P1.Z, B);
...
int EdwardsTripleAtomicity(projectPoint P1, extProjectPoint &P3, big d)
{
	...
	nres_modmult(P1.Y, P1.Y, YY);
	//Y_1^2
	nres_modadd(YY, YY, A);//Atomicity, it does not affect the final value
	nres_modadd(YY, YY, A);//Atomicity, it does not affect the final value
...
int EdwardsAdditionSubtractionAtomicity(extProjectPoint P1, extProjectPoint P2, extProjectPoint &P3, extProjectPoint &P4, big d)
{
	...
	//A1=H*A1=(Y1-X1)(Y2+X2)
	nres_modadd(A1, A1, A);//Atomicity, it does not affect the final value
	nres_modadd(A1, A1, A);//Atomicity, it does not affect the final value
	...
	nres_modmult(H, I, B1);
	//B1=H*I=(Y1+X1)(Y2-X2)
	nres_modadd(P2.Z, P2.Z, H);
	nres_modadd(A1, A1, F);//Atomicity, it does not affect the final value

	nres_modmult(P1.Z, P2.T, C);
	nres_modadd(C, C, C);//C=2Z1*T2
	nres_modsub(zero, C, C1);
	//C1=-C

	
	nres_modmult(P1.T, H, D);
	//D=2T1*Z2
	nres_modadd(D, C, E);//E=D+C
	nres_modsub(B, A, F);//F=B-A
...
int InvEdwardsMixedAdditionAtomicity(projectPoint P1, affinePoint P2, projectPoint &P3, big d)
{
	...
	big A = mirvar(0), C = mirvar(0), B = mirvar(0), E = mirvar(0), H = mirvar(0), I = mirvar(0), P3X = mirvar(0);
	...
	nres_modmult(C, I, E);	//Big E=(C*D)%p;//C*D
...
int InvEdwardsAdditionAtomicity(projectPoint P1, projectPoint P2, projectPoint &P3, big d,big a)
{
	...
	big A = mirvar(0), D = mirvar(0), F = mirvar(0), G = mirvar(0);

	//Big A=P1.Z;//Z_1Z_2,Z_2=1
	nres_modmult(P1.Z, P2.Z, A);
	nres_modadd(A, A, G);//Atomicity, it does not affect the final value
	nres_modadd(A, A, G);//Atomicity, it does not affect the final value
...
{
	big A=mirvar(0),B=mirvar(0),C=mirvar(0),D=mirvar(0),E=mirvar(0), G = mirvar(0);
	nres_modmult(P1.X,P1.X,A);
	//Big A=(P1.X*P1.X)%p;//X_1^2
	nres_modadd(A, A, G);//Atomicity, it does not affect the final value
	nres_modadd(A, A, G);//Atomicity, it does not affect the final value
...
int InvEdwardsTripleAtomicity(projectPoint P1, projectPoint &P3, big d)
{
	...
	big A=mirvar(0),B=mirvar(0),C=mirvar(0),D=mirvar(0),H=mirvar(0),P=mirvar(0),Q=mirvar(0), G = mirvar(0);
...
int DIKMixedAdditionAtomicity(extProjectPoint P1, affinePoint P2, extProjectPoint &P3)
{
	//T=ZZ,.T is .ZZ
	//7M+4S
	big C = mirvar(0), B = mirvar(0), E = mirvar(0), H = mirvar(0), I = mirvar(0), J = mirvar(0);
	big a = mirvar(6), four = mirvar(4), zero = mirvar(0);//a=3a=3*2,is curve parameter
	...
	nres_modsub(P1.X, A, C);
	//C=X1-A
	nres_modadd(A, A, J);//Atomicity, it does not affect the final value
...
int DIKAdditionAtomicity(extProjectPoint P1, extProjectPoint P2, extProjectPoint &P3)
{
	//T=ZZ,.T is .ZZ
	//11M+6S
	big C = mirvar(0), B = mirvar(0), E = mirvar(0), H = mirvar(0), I = mirvar(0), J = mirvar(0), P3X = mirvar(0);
	big a = mirvar(6), four = mirvar(4), eight = mirvar(8), sixteen = mirvar(16), zero = mirvar(0);//a=3a=3*2,is curve parameter
	big A = mirvar(0), D = mirvar(0), F = mirvar(0), F4 = mirvar(0), G = mirvar(0), H2 = mirvar(0);
...
int DIKAdditionSubtractionAtomicity(extProjectPoint P1, extProjectPoint P2, extProjectPoint &P3, extProjectPoint &P4)
{
	...
	nres_modmult(P2.Z, P2.T, ZZZ2);
	//ZZZ2=Z2*ZZ2
	nres_modadd(ZZZ2, A, A);//Atomicity, it does not affect the final value
	nres_modadd(A, A, J);//Atomicity, it does not affect the final value
...
int DIKDoublingAtomicity(extProjectPoint P1,extProjectPoint &P3)
{
	...
	nres_modmult(P3.Z, P3.Z,P3.T);	//T3=Z3^2
...
int DIKTriplingAtomicity(extProjectPoint P1, extProjectPoint &P3)
{
	...
	//YY1=Y1^2
	nres_modadd(A, YY1, A);//Atomicity, it does not affect the final value
	nres_modadd(A, A, J);//Atomicity, it does not affect the final value
