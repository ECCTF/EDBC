...
int MontgomeryAddition(affinePoint P1, affinePoint P2, MontgomeryPoint &P3, big A)
{
	big slope = mirvar(1);
	big R1 = mirvar(0), R2 = mirvar(0), M = mirvar(0), one =mirvar(1);//
	nres_modsub(P1.x, P2.x, R1);
	nres_modsub(P1.y, P2.y, R2);
	nres_moddiv(R2, R1, slope);
	nres_modmult(slope, slope, M);
	nres_modsub(M, A, R1);
	nres_modsub(R1, P1.x, R1);
	nres_modsub(R1, P2.x, R2);//R2=x(P1-P2)

	nres_moddiv(one, R2, P3.X);
	copy(one, P3.Z);

	return 0;
}
int invMontgomeryAddition(MontgomeryPoint P1, MontgomeryPoint P2, MontgomeryPoint &P3, big A)
{
...
{//P1=P3-P2=P5-P4,ConstantA=(A-2)/4
	...
	nres_modmult(C, C, A);//A=(X2+Z2)^2
	nres_modmult(D, D, B);//B=(X2-Z2)^2
	nres_modmult(A, B, P4.X);//P4.X=(X2+Z2)^2(X2-Z2)^2
	nres_modsub(A, B, E);//E=(X2+Z2)^2-(X2-Z2)^2
	nres_modmult(E, ConstantA, F);//F=(A-2)/4*((X2+Z2)^2-(X2-Z2)^2)
	nres_modadd(A, F, G);//G=(X2+Z2)^2+(A-2)/4*((X2+Z2)^2-(X2-Z2)^2)
	nres_modmult(E, G, P4.Z);//P4.Z=((X2+Z2)^2-(X2-Z2)^2)((X2+Z2)^2+(A-2)/4*((X2+Z2)^2-(X2-Z2)^2))

	nres_modadd(P3.X, P3.Z, A);//A=X3+Z3
	nres_modsub(P3.X, P3.Z, B);//B=X3-Z3

	nres_modmult(D, A, E);//E=(X2-Z2)(X3+Z3)
	nres_modmult(B, C, F);//F=(X2+Z2)(X3-Z3)


	nres_modadd(E, F, A);//A=(X2-Z2)(X3+Z3)+(X2+Z2)(X3-Z3)
	nres_modsub(E, F, B);//B=(X2-Z2)(X3+Z3)-(X2+Z2)(X3-Z3)

	nres_modmult(A, A, E);//E=((X2-Z2)(X3+Z3)+(X2+Z2)(X3-Z3))^2
	nres_modmult(B, B, F);//F=((X2-Z2)(X3+Z3)-(X2+Z2)(X3-Z3))^2
	nres_modmult(E, P1.Z, P5.X);//P5.X=((X2-Z2)(X3+Z3)+(X2+Z2)(X3-Z3))^2 Z1
	nres_modmult(F, P1.X, P5.Z);//P5.Z=((X2-Z2)(X3+Z3)-(X2+Z2)(X3-Z3))^2 X1
	return 0;
}
